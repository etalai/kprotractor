...
export class DatabasesPage extends ManageDataPage {
  ...
  waitForDBTableLoad(automationtableID): promise.Promise<any> {
    ...
    return browser.wait(until.presenceOf(this.getLoadDataBtnByTableName(automationtableID)), 40 * 1000,
      `Table ${automationtableID} should be loaded in 40 secs`);
  }
  ...
  navigateToViewEditDB(databaseName): promise.Promise<any> {
    ...
    return browser.wait(until.presenceOf(this.getLoadDataBtnByTableName('school_directory')), 40 * 1000,
      'Database details page should be loaded in 40 secs');
  }

  loadData(tablename, filename): promise.Promise<any> {
    const until = protractor.ExpectedConditions;
    browser.wait(until.presenceOf(this.getLoadDataBtnByTableName(tablename)), 40 * 1000,
      'Database details page should be loaded in 40 secs');
    ...
    browser.wait(until.presenceOf(this.loadDataJobList), 40 * 1000,
      'Load Jobs page should be loaded in 40 secs');
