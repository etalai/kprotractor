...
defineSupportCode(({ Given, When, Then, Before, After }) => {
    let login: ApplicationSSOPage;
    // tslint:disable-next-line:no-require-imports
    const userDataConfig = require('../../testdata/e2e.test.sso.json');

    Before(() => {
        login = new ApplicationSSOPage();
    });
    ...
    Then('Then I see the {string} error message', async errorMessage => {
